...
ios: target/universal/release/libcore.a

target/universal/release/libcore.a: $(SOURCES) ndk-home
...
android: target/aarch64-linux-android/release/libcore.so target/armv7-linux-androideabi/release/libcore.so target/i686-linux-android/release/libcore.so target/x86_64-linux-android/release/libcore.so

target/aarch64-linux-android/release/libcore.so: $(SOURCES) ndk-home
	CC_aarch64_linux_android=$(ANDROID_AARCH64_LINKER) \
	CARGO_TARGET_AARCH64_LINUX_ANDROID_LINKER=$(ANDROID_AARCH64_LINKER) \
		cargo build --target aarch64-linux-android --release
	@echo "[DONE] $@"

target/armv7-linux-androideabi/release/libcore.so: $(SOURCES) ndk-home
...
target/i686-linux-android/release/libcore.so: $(SOURCES) ndk-home
	CC_i686_linux_android=$(ANDROID_I686_LINKER) \
	CARGO_TARGET_I686_LINUX_ANDROID_LINKER=$(ANDROID_I686_LINKER) \
		cargo  build --target i686-linux-android --release
	@echo "[DONE] $@"

target/x86_64-linux-android/release/libcore.so: $(SOURCES) ndk-home
